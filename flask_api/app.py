...
cred = credentials.Certificate("ibm-safety-net-f2a46-firebase-adminsdk-r4n7a-f3c456d02e.json")
...
cache = Cache(app)
video_path = "C:/Users/Roman/PycharmProjects/Video Analytics Industrial Safety/videos/vid1.mp4"
        ...
        if self.user_type == "managers":
            ...
            for docs in data_ref:
                ...
                for items in docs.to_dict()['data']:
                    score = (int(items['safety-vest']) + int(items['hard-hat']) + int(items['mask'])) / (
                                3 * int(items['person']))
                    avg = avg + score
...
def get_data():
    ...
    try:
        ...
        if user_type == "managers":
            sites_ref = db.collection("sites").where(filter=FieldFilter("manager_id", "==", user_id))
            site = {}
            ...
            camera_ref = (
                db.collection("cameras").where(filter=FieldFilter("site_id", "==", site["site_id"])).get()
            )
            cam = []
            cam_ref = {}
            ...
            sensor_ref = (
                db.collection("sensor").where(filter=FieldFilter("site_id", "==", site["site_id"])).get()
            )
            sensor_data = []
            ...
            fire_detection_ref = db.collection("fire-detection").where(
                filter=FieldFilter("site_id", "==", site["site_id"])
            )
            fire_detection_data = []
            ...
            sites_ref = db.collection("sites").where(filter=FieldFilter("admin_id", "==", user_id))
            sites = []
            site_ids = {}
            ...
            managers_ref = db.collection("managers").where(filter=FieldFilter("admin_id", "==", user_id))
            managers = []
            ...
            cameras_ref = db.collection("cameras").where(
                filter=FieldFilter("site_id", "in", list(site_ids.keys()))
            )
            cam = []
            cam_ref = {}
...
def safety_gear():
    ...
    try:
        ...
        doc_ref.update({"data": ArrayUnion([temp])})
...
def hand_gesture():
    ...
    try:
        ...
        send_email(sender_email='coc55228@gmail.com', sender_password='jpdtwuyrdvdovban',
                   receiver_email='divija.kinger@somaiya.edu', subject='Person in Danger!',
                   message=f'A person is in danger at location {camera_data["location"]}. Please check the video at {video_link}')
...
def fire_alert():
    ...
    try:
        ...
        ws.emit("notification", data)
        send_email(sender_email='coc55228@gmail.com', sender_password='jpdtwuyrdvdovban',
                   receiver_email='divija.kinger@somaiya.edu', subject='Fire Alert!',
                   message=f'There is a fire at location {camera_data["location"]}. Please check the video at {video_link}')
        ...
        if reading == True:
            if cache.get("sensor-cache") == None:
                ...
                if cache.get("sensor-cache") >= 5:
                    ...
                    ws.emit("notification", data)
                    send_email(sender_email='coc55228@gmail.com', sender_password='jpdtwuyrdvdovban',
                               receiver_email='divija.kinger@somaiya.edu', subject='Gas Leak!',
                               message=f'Gas leak at location {sensor_data["location"]}')
...
def statistics():
    ...
    return jsonify(
        {"safety_gear_data": safety_gear_data, "sensor_fire_data": sensor_fire_data, 'safety_avg_data': avg_data})
